...
<head>
  ...
  <title>TeamAMovie - Admin | Dashboard</title>
            ...
            <div class="card-body">
                ...
                <div id="output" class="table-responsive">
                    <table id="table" class="table table-striped table-bordered table-sm">
                        ...
                        <tbody>
                            <?php
                                $sql = "SELECT A.* FROM tbl_refunds A, tbl_bookings B, tbl_shows C, tbl_theatres D WHERE A.ticket_id = B.ticket_id AND B.show_id = C.show_id AND C.theatre_id = D.theatre_id AND D.admin_id = '{$_SESSION['admin_id']}' GROUP BY B.ticket_id ORDER BY transaction_id DESC";
                                $result = $db->query($sql);
                                if($result->num_rows>0){
                                    ...
                                    while($row=$result->fetch_assoc()){
                                        ...
                                        echo "<tr style='text-align:center;'>";
                                            ...
                                            echo "</td>";
                                            echo "<td style='vertical-align:middle'>RMB ".number_format((float)$row['amount'], 2, '.', '')."</td>";
                                            echo "<td style='vertical-align:middle'>";
                                                if($row['status'] == '0'){
                                            ...
                                            echo "</td>";
                                            echo "<td style='vertical-align:middle' id='action_column'>";
                                                if($row['status'] == '0'){
                                                    echo "<a href='assets/refunds.confirm.php?transaction_id=".$row['transaction_id']."'><button type='button' align='center' class='btn btn-danger btn-sm'>Confirm</button></a>";
                                                }else if($row['status'] == '1'){
                                                    echo "<span style='font-weight:bold;font-size:15px;color:green'>Paid</span>";
                                                }
                                            echo "</td>";
                                        echo "</tr>";
                                    }
...
<script>
    $(".sidebar-fixed .list-group .list-group-item.item-10").addClass("active");
</script>
